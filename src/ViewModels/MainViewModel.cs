...
public class MainViewModel : ObservableObject
{
    ...
    public ObservableCollection<Dlss5ChainReader.ChainLink> Dlss5Chain { get; } = new();
    ...
    private async Task PreaquecerDetalhe(GameItemVm item, CancellationToken ct)
    {
        ...
        try
        {
            ...
            await LerPasta($"cadeia\u0001{exe}\u0001{aplicado}", dir,
                () => Dlss5ChainReader.LerCadeia(dir, ini, det, exe, aplicado));
    ...
    private async Task CheckNeuralAsync(int token)
    {
        ...
        var leitura = await LerPasta($"cadeia\u0001{exeDaVez}\u0001{aplicadoAgora}", targetDir,
            () => Dlss5ChainReader.LerCadeia(targetDir, neuralIni, detection, exeDaVez, aplicadoAgora));
        if (token != _detailToken) return;
                                    ...
                                    && NeuralUpliftService.ReassertEnabled(neuralIni)))
        {
            ...
            AplicarCadeia(await Task.Run(() =>
                Dlss5ChainReader.LerCadeia(targetDir, neuralIni, detection, exeDaVez, NeuralApplied)));
